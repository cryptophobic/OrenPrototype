...
class ActorsContext(UserDict[str, Actor]):
    def __init__(self, actors_collection: ActorsCollection):
        super().__init__()
        for actor in actors_collection.data.values():
    ...
    def get_coordinate_holders(self) -> CoordinateHoldersCollection:
        return CoordinateHoldersCollection(self.__get_by_type(CoordinateHolder))

    def get_static_objects(self) -> StaticObjectsCollection:
        return StaticObjectsCollection(self.__get_by_type(StaticObject))

    def get_units(self) -> UnitsCollection:
        return UnitsCollection(self.__get_by_type(Unit))

    def get_actors(self) -> ActorsCollection:
        return ActorsCollection(self.data.copy())
